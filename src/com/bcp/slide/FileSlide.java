...
public class FileSlide {
    ...
    public static void delete(File file) {
        ...
        if (file.isFile()) {
            Logger.debug("Archivo eliminado - delete" + file.delete());
            return;
        }
        ...
        if (file.isDirectory()) {
            ...
            if (childFiles == null || childFiles.length == 0) {
                Logger.debug( "Archivo eliminado - delete" + childFiles);
                return;
            }
    ...
    public static void copyToAssets2(Context context, String assetDir, String dir) {
        ...
        try {
            ...
            Logger.logLine(Logger.LOG_EXECPTION, e1.getStackTrace(), " FileSlide " +  e1.getMessage());
            return;
        }
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if(fileName !=null && !fileName.contains(".")){
                if (0 == assetDir.length()) {
                    ...
                    copyToAssets(context, assetDir + "/" + fileName, dir + "/" + fileName + "/");
                }
                    continue;
            }
            File outFile = new File(mWorkingPath, fileName);
            if (outFile.exists()) {
                Logger.debug( "Archivo eliminado - copyToAssets2" +outFile.delete());
            }
            InputStream in = null;
            try (FileOutputStream out = new FileOutputStream(outFile)){
                ...
                try {
                    ...
                    Logger.logLine(Logger.LOG_EXECPTION, e.getStackTrace(), " FileSlide 1 " +  e.getMessage());
    ...
    public static void copyToAssets(Context context, String assetDir, String dir) {
        ...
        try {
            ...
            Logger.logLine(Logger.LOG_EXECPTION, e1.getStackTrace(), " FileSlide 2 " +  e1.getMessage());
            return;
        }
        File mWorkingPath = new File(dir);
        // 如果文件路径不存在
        ...
        for (int i = 0; i < files.length; i++) {
            try {
                ...
                try {
                    ...
                    Logger.logLine(Logger.LOG_EXECPTION, e.getStackTrace(), " FileSlide 3 " +  e.getMessage());
                    //it said that this is a directory
                    if (0 == assetDir.length()) {
                        copyToAssets(context, fileName, dir + fileName + "/");
                    } else {
                ...
                File outFile = new File(mWorkingPath, fileName);
                if (outFile.exists()) {
                    Logger.debug("Archivo eliminado - copyToAssets" + outFile.delete());
                }
                ...
                Logger.logLine(Logger.LOG_EXECPTION, e.getStackTrace(), " FileSlide 4 " +  e.getMessage());
