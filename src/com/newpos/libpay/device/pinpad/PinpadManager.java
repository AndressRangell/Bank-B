...
public class PinpadManager {
     ...
     * @param l
     */
    public void getPin(String amount , String c , String tittle, final PinpadListener l){
        this.listener = l ;
        this.pinCardNo = c ;
        final PinInfo info = new PinInfo();
        if(pinCardNo == null || pinCardNo.equals("")){
            ...
            if(Locale.getDefault().getLanguage().equals("ZH")){
                padView.setTitleMsg("华智融安全键盘");//TITULO DE LA PANTALLA
                padView.setAmountTitle("金额:");//PRIMER MENSAJE CON VALOR DEL MONTO
                padView.setAmount(PAYUtils.twoWei(amount));//MONTO
                padView.setPinTips("请输入密码:");//MENSAJE PARA EL USUARIO
            }else {
                padView.setPinTips(tittle);
            }
            ...
            new Thread(){
                ...
                public void run() {
                    ped.getPinBlock(KeySystem.MS_DES,
                            MASTERKEYIDX,
                            PinBlockFormat.PIN_BLOCK_FORMAT_0, NUMBER, pinCardNo,
                            (i, bytes) -> {
                                switch (i){
                                    ...
                                    case PedRetCode.ENTER_CANCEL:
                                        info.setResultFlag(false);
                                        info.setErrno(TcodeError.T_USER_CANCEL_PIN_ERR);
                                        break;
                                    case 0:
                                        info.setResultFlag(true);
    ...
    public void getOfflinePin(int i , OfflineRSA key , int counts , PinpadListener l){
        ...
        ped.getOfflinePin(i == 1 ? KeySystem.ICC_CIPHER:KeySystem.ICC_PLAIN,
                ped.getIccSlot(SlotType.USER_CARD), NUMBER, apdu,
    ...
    public byte[] getMac(byte[] data, int offset, int len) {
        ...
        return Ped.getInstance().getMac(KeySystem.MS_DES, TMConfig.getInstance().getMasterKeyIndex(), MACMode.MAC_MODE_CUP_8, macIn);
    }
    ...
    public byte[] getCITICMac(byte[] data, int offset, int len) {
        ...
        macIn = new byte[((len + 7) >> 3) << 3];
        System.arraycopy(data, offset, macIn, 0, len);
        return Ped.getInstance().getMac(KeySystem.MS_DES, TMConfig.getInstance().getMasterKeyIndex(), MACMode.MAC_MODE_CUP, macIn);
    }
    ...
    public String getEac(int index , String track) {
        int ofs;
        int orgLen;
        StringBuilder trackEnc = new StringBuilder(120);
        byte[] bufSrc ;
        byte[] bufDest ;
        if (track == null || track.equals("")) {
            return null;
        }
        orgLen = track.length();//37
        if (((orgLen % 2) != 0)) {
            if (track.length() < 17) {
                return null;
            }
        ...
        trackEnc.append(track.substring(ofs + 16, orgLen));
     ...
     * @return
     */
    public String getDocCipher(int index, String document){
        byte[] bufSrc ;
        byte[] bufDest ;
        bufSrc = ISOUtil.str2bcd(document, false);
        bufDest = Ped.getInstance().encryptAccount(KeySystem.MS_DES, index, Ped.TDEA_MODE_ECB, bufSrc);
        if ( bufDest == null ) {
            return null;
        }
        return ISOUtil.byte2hex(bufDest);
    }
     ...
     * @return
     */
    public int getOfflinePin(int keyType, RsaPinKey rsaPinkey, final byte[] len, final byte[] offlinePinBlcok) {
        Ped ped = Ped.getInstance() ;
        PadView padView = new PadView();
        try {
        ...
        status.set(false);
        mErrorCode = -1;
        ped.getOfflinePin(ks, fd, NUMBER, apdu, new PinBlockCallback() {
            @Override
        ...
        tryLock(offlinePinLock, TIMEOUT);
    ...
    private void tryLock(Object lock, int timeOut) {
        ...
        if(lock != null) {
            try {
                ...
                Logger.logLine(Logger.LOG_EXECPTION, e.getStackTrace(), " PinpadManager " +  e.getMessage());
     ...
     */
    private static final int TIMEOUT = 70;//Second
}
