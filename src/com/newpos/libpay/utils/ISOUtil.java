...
public class ISOUtil {
    ...
    protected static final String MSG = "Number can have maximum 9 digits";
    ...
    public static BitSet byte2BitSet(byte[] b, int offset,
        ...
        int len;
        if (bitZeroMeansExtended) len = (b[offset] & 0x80) == 0x80 ? 128 : 64;
        else len = 64;
    ...
    public static BitSet byte2BitSet(byte[] b, int offset, int maxBits) {
        int len;
        if (maxBits > 64) len = (b[offset] & 0x80) == 0x80 ? 128 : 64;
        else len = maxBits;
    ...
    public static BitSet hex2BitSet(byte[] b, int offset,
                                    boolean bitZeroMeansExtended) {
        int len;
        if (bitZeroMeansExtended) len = (Character.digit((char) b[offset], 16) & 0x08) == 8 ? 128
                : 64;
        else len = 64;
        BitSet bmap = new BitSet(len);
        for (int i = 0; i < len; i++) {
    ...
    public static BitSet hex2BitSet(byte[] b, int offset, int maxBits) {
        int len;
        if (maxBits > 64) len = (Character.digit((char) b[offset], 16) & 0x08) == 8 ? 128
                : 64;
        else len = maxBits;
