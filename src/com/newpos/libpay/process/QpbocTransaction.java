...
public class QpbocTransaction {
    ...
    private long qAmount;
    private long qOtheramount;
    ...
    public QpbocTransaction(TransInputPara p){
        ...
        if(para.isNeedAmount()){
            qAmount = para.getAmount();
            qOtheramount = para.getOtherAmount();
        }
    ...
    public int start(){
        ...
        int transType  = 0 ;
        int amount  = Integer.parseInt(String.valueOf(para.getAmount())) ;
        ...
        if(para.isECTrans()){
            ...
            Logger.debug("取电子现金余额（9F79）="+ ecAmount);
        }
        ret = qPbocHandler.readData(0);
        Logger.debug("qpboc.readData ret = "+ret);
        if(ret!=0){
            return TcodeError.T_QPBOC_ERRNO;
        }
        return ret ;
    }
    ...
    private IEMVCallback.EMVInitListener emvInitListener = new IEMVCallback.EMVInitListener() {
        ...
        public int getAmount(int[] transAmount, int[] cashBackAmount) {
            ...
            Logger.debug("====getAmount======"+ qAmount);
            if (para.isNeedAmount()) {
                if(qAmount <= 0){
                    // 调用输入金额
                    Logger.debug("EMV>>需要输入金额且金额参数未0>>EMV流程执行获取金额的回调");
                    InputInfo info = transUI.getOutsideInput(UIUtils.fillScreenAmount("", "1200", "AHORROS", timeout, "S/.","INGRESE MONTO", "200"));
                    if(info.isResultFlag()){
                        qAmount = (int) (Double.parseDouble(info.getResult()) * 100);
                        Logger.debug("EMV>>用户输入的金额="+ qAmount);
                    }
                    qOtheramount = 0;
                }else {
                    transAmount[0] = Integer.valueOf(qAmount + "");
                    cashBackAmount[0] = Integer.valueOf(qOtheramount + "");
                }
            }
            return 0;
        }
    ...
    public boolean initEmvKernel() {
        ...
        configure.setTerminalCapabilities(new byte[] { (byte) 0xE0, (byte) 0xF0, (byte) 0xC8 });
        ...
        configure.setSupportForcedOnlineCapability(para.isNeedOnline());
